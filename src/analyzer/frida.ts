...
export interface MemoryReadOptions {
  ...
  type: 'utf8' | 'utf16' | 'ansi' | 'pointer' | 'int32' | 'uint32' | 'int64' | 'uint64' | 'float' | 'double' | 'byteArray';
...
export class FridaGenerator {
  ...
  public static generateInterceptor(opts: InterceptorOptions): string {
    ...
    if (typeof opts.target === 'number') {
      ...
      if (opts.libraryName) {
        lines.push(`const moduleBase = Module.findBaseAddress("${opts.libraryName}");`);
      ...
      if (opts.libraryName) {
        lines.push(`const targetAddress = Module.findExportByName("${opts.libraryName}", "${opts.target}");`);
        lines.push(`if (targetAddress) {`);
        lines.push(`  Interceptor.attach(targetAddress, {`);
      } else {
        lines.push(`const targetAddress = Module.findExportByName(null, "${opts.target}");`);
        lines.push(`if (targetAddress) {`);
        lines.push(`  Interceptor.attach(targetAddress, {`);
      }
    }

    const indent = typeof opts.target === 'number' && !opts.libraryName ? '' : '  ';
    ...
    if (opts.logBacktrace) {
      lines.push(`${indent}    console.log("Backtrace:\\n" + Thread.backtrace(this.context, Backtracer.ACCURATE).map(DebugSymbol.fromAddress).join("\\n"));`);
    }
    ...
    if (opts.logArguments && opts.args && opts.args.length > 0) {
      opts.args.forEach((arg, idx) => {
        if (arg.type === 'string') {
          ...
          lines.push(`${indent}      console.log("  arg[${idx}] (${arg.name}): " + Memory.readUtf8String(args[${idx}]));`);
          lines.push(`${indent}    } catch (e) {`);
          lines.push(`${indent}      console.log("  arg[${idx}] (${arg.name}): " + args[${idx}] + " (unable to read string)");`);
          lines.push(`${indent}    }`);
        } else if (arg.type === 'int') {
          lines.push(`${indent}    console.log("  arg[${idx}] (${arg.name}): " + args[${idx}].toInt32());`);
        } else if (arg.type === 'float') {
          lines.push(`${indent}    console.log("  arg[${idx}] (${arg.name}): " + args[${idx}]);`); // standard pointer dump/float interpretation
        } else {
          lines.push(`${indent}    console.log("  arg[${idx}] (${arg.name}): " + args[${idx}]);`);
        }
      });
    }

    if (opts.customOnEnter) {
      const indentedCustom = opts.customOnEnter.split('\n').map(l => `${indent}    ${l}`).join('\n');
    ...
    if (opts.logReturnValue) {
      ...
      if (retType === 'void') {
        ...
        lines.push(`${indent}    try {`);
        lines.push(`${indent}      console.log("[+] ${name} returned string: " + Memory.readUtf8String(retval));`);
        lines.push(`${indent}    } catch (e) {`);
        lines.push(`${indent}      console.log("[+] ${name} returned: " + retval + " (unable to read string)");`);
        lines.push(`${indent}    }`);
      } else if (retType === 'int') {
        lines.push(`${indent}    console.log("[+] ${name} returned int: " + retval.toInt32());`);
      } else {
        lines.push(`${indent}    console.log("[+] ${name} returned: " + retval);`);
      }
    } else {
    ...
    if (opts.customOnLeave) {
      const indentedCustom = opts.customOnLeave.split('\n').map(l => `${indent}    ${l}`).join('\n');
      lines.push(indentedCustom);
    }
    lines.push(`${indent}  }`);
    lines.push(`${indent}});`);
    ...
    if (opts.libraryName || typeof opts.target !== 'number') {
      lines.push(`} else {`);
      lines.push(`  console.log("[-] Target function ${opts.target} not found");`);
      lines.push(`}`);
    }
    ...
    switch (opts.type) {
      case 'utf8':
        if (opts.length !== undefined) {
          lines.push(`  const ${varName} = Memory.readUtf8String(${opts.addressExpression}, ${opts.length});`);
        } else {
          lines.push(`  const ${varName} = Memory.readUtf8String(${opts.addressExpression});`);
        }
        break;
      case 'utf16':
        if (opts.length !== undefined) {
          lines.push(`  const ${varName} = Memory.readUtf16String(${opts.addressExpression}, ${opts.length});`);
        } else {
          lines.push(`  const ${varName} = Memory.readUtf16String(${opts.addressExpression});`);
        }
        break;
      case 'ansi':
        if (opts.length !== undefined) {
          lines.push(`  const ${varName} = Memory.readAnsiString(${opts.addressExpression}, ${opts.length});`);
        } else {
          lines.push(`  const ${varName} = Memory.readAnsiString(${opts.addressExpression});`);
        }
        break;
      case 'pointer':
        lines.push(`  const ${varName} = Memory.readPointer(${opts.addressExpression});`);
        break;
      case 'int32':
        lines.push(`  const ${varName} = Memory.readS32(${opts.addressExpression});`);
        break;
      case 'uint32':
        lines.push(`  const ${varName} = Memory.readU32(${opts.addressExpression});`);
        break;
      case 'int64':
        lines.push(`  const ${varName} = Memory.readS64(${opts.addressExpression});`);
        break;
      case 'uint64':
        lines.push(`  const ${varName} = Memory.readU64(${opts.addressExpression});`);
        break;
      case 'float':
        lines.push(`  const ${varName} = Memory.readFloat(${opts.addressExpression});`);
        break;
      case 'double':
        lines.push(`  const ${varName} = Memory.readDouble(${opts.addressExpression});`);
        break;
      case 'byteArray':
        const len = opts.length || 16;
        lines.push(`  const ${varName} = Memory.readByteArray(${opts.addressExpression}, ${len});`);
        break;
      default:
        lines.push(`  const ${varName} = Memory.readPointer(${opts.addressExpression});`);
    }
    
    lines.push(`  console.log("[*] Read ${varName}: " + ${opts.type === 'byteArray' ? `hexdump(${varName})` : varName});`);
    lines.push(`} catch (e) {`);
    lines.push(`  console.log("[-] Failed to read memory at " + ${opts.addressExpression} + ": " + e);`);
    lines.push(`}`);
  ...
  public static generateRegisterDump(opts: RegisterDumpOptions): string {
    ...
    if (opts.registers && opts.registers.length > 0) {
      ...
      switch (opts.architecture) {
        case 'x64':
          targetRegs = ['rax', 'rbx', 'rcx', 'rdx', 'rsi', 'rdi', 'rsp', 'rbp', 'rip', 'r8', 'r9', 'r10', 'r11'];
          break;
        case 'x86':
          targetRegs = ['eax', 'ebx', 'ecx', 'edx', 'esi', 'edi', 'esp', 'ebp', 'eip'];
          break;
        case 'arm64':
          targetRegs = ['x0', 'x1', 'x2', 'x3', 'x4', 'x8', 'x16', 'x29', 'x30', 'sp', 'pc'];
          break;
        case 'arm':
          targetRegs = ['r0', 'r1', 'r2', 'r3', 'r4', 'r7', 'r11', 'r12', 'sp', 'lr', 'pc'];
          break;
      }
    }
    ...
    targetRegs.forEach(reg => {
      lines.push(`try {`);
      lines.push(`  console.log("  ${reg.padEnd(5)}: " + this.context.${reg});`);
      lines.push(`} catch (e) {`);
      lines.push(`  console.log("  ${reg.padEnd(5)}: unavailable (" + e + ")");`);
      lines.push(`}`);
    });
  ...
  public static generateJavaHook(opts: JavaHookOptions): string {
    ...
    const overloadStr = opts.overloadTypes && opts.overloadTypes.length > 0
      ? opts.overloadTypes.map(t => `"${t}"`).join(', ')
      : '';
    const overloadAccessor = overloadStr ? `.overload(${overloadStr})` : '';
    
    lines.push(`    targetClass["${opts.methodName}"]${overloadAccessor}.implementation = function (...args) {`);
    lines.push(`      console.log("[+] Java ${opts.className}.${opts.methodName} called");`);
    ...
    if (opts.customImplementation) {
      const indented = opts.customImplementation.split('\n').map(l => `      ${l}`).join('\n');
      lines.push(indented);
    } else {
      lines.push(`      const retval = this["${opts.methodName}"]${overloadAccessor}(...args);`);
      if (opts.logReturnValue) {
    ...
    lines.push(`    };`);
    lines.push(`  } catch (e) {`);
    lines.push(`    console.log("[-] Java hook error for ${opts.className}.${opts.methodName}: " + e);`);
    lines.push(`  }`);
    lines.push(`});`);
  ...
  public static generateObjcHook(opts: ObjcHookOptions): string {
    ...
    lines.push(`        console.log("[+] ObjC " + className + " " + selectorName + " called");`);
    
    if (opts.logArguments) {
      lines.push(`        // ObjC methods have self at index 0 and _cmd selector at index 1`);
      lines.push(`        console.log("  self: " + args[0]);`);
      lines.push(`        console.log("  _cmd: " + ObjC.selectorAsString(args[1]));`);
