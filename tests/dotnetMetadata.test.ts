...
import {
  DotNetMetadataParser,
  readCompressedUint32,
} from '../src/parser/dotnetMetadata.js';
...
describe('.NET Metadata Parser Unit Tests', () => {
  it('should parse compressed uint32 correctly', () => {
    ...
    expect(readCompressedUint32(view1, 0)).toEqual({
      value: 0x3f,
      bytesRead: 1,
    });
    ...
    expect(readCompressedUint32(view2, 0)).toEqual({
      value: (0x1f << 8) | 0x90,
      bytesRead: 2,
    });
    ...
    expect(readCompressedUint32(view3, 0)).toEqual({
      value: (0x0a << 24) | (0x1b << 16) | (0x2c << 8) | 0x3d,
      bytesRead: 4,
    });
  });
  ...
  it('should parse metadata tables correctly from #~ stream', () => {
    ...
    for (let i = 0; i < strName.length; i++)
      view.setUint8(40 + i, strName.charCodeAt(i));
    ...
    for (let i = 0; i < tablesName.length; i++)
      view.setUint8(60 + i, tablesName.charCodeAt(i));
    ...
    for (let i = 0; i < classStr.length; i++)
      view.setUint8(stringsBase + 1 + i, classStr.charCodeAt(i));
