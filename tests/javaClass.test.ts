...
describe('Java Class Parser Unit Tests', () => {
  ...
  it('should throw an error for invalid magic number', () => {
    ...
    expect(() => parseJavaClass(buffer)).toThrow(
      'Invalid Java Class Magic: 0xDEADBEEF'
    );
  });
  ...
  it('should parse constant pool entries including Long and Double double-slot tags', () => {
    ...
    offset += 2; // cp count is 8

    // CP[1]: Integer (value 123456)
    bytes[offset] = 3;
    offset += 1;
    view.setUint32(offset, 123456, false);
    offset += 4;

    // CP[2]: Float (value 1.25)
    bytes[offset] = 4;
    offset += 1;
    view.setFloat32(offset, 1.25, false);
    offset += 4;

    // CP[3]: Long (value 0x1122334455667788n) - takes 2 slots (CP[3], CP[4])
    bytes[offset] = 5;
    offset += 1;
    view.setBigUint64(offset, 0x1122334455667788n, false);
    offset += 8;

    // CP[5]: Double (value 3.14159) - takes 2 slots (CP[5], CP[6])
    bytes[offset] = 6;
    offset += 1;
    view.setFloat64(offset, 3.14159, false);
    offset += 8;
    ...
    offset += 2; // this class
    view.setUint16(offset, 0, false);
    offset += 2; // super class
    view.setUint16(offset, 0, false);
    offset += 2; // interfaces count
    view.setUint16(offset, 0, false);
    offset += 2; // fields count
    view.setUint16(offset, 0, false);
    offset += 2; // methods count
    view.setUint16(offset, 0, false);
  ...
  it('should successfully parse fields, methods, and bytecode attributes', () => {
    ...
    offset += 2;
    for (let j = 0; j < 15; j++)
      bytes[offset++] = 'LineNumberTable'.charCodeAt(j);
    ...
    offset += 4; // code_length
    bytes[offset++] = 0x2a; // aload_0
    bytes[offset++] = 0xb1; // return
    view.setUint16(offset, 0, false);
    offset += 2; // exception_table_length
    view.setUint16(offset, 1, false);
    ...
    offset += 4; // length
    view.setUint16(offset, 1, false);
    offset += 2; // sourcefile_index CP[1] -> "ClassA"
    ...
    const lnTable = codeAttr?.decoded.attributes.find(
      (a: any) => a.name === 'LineNumberTable'
    );
  ...
  it('should correctly format access flags for classes, fields, and methods', () => {
    ...
    const fieldFlags = formatAccessFlags(
      0x0002 | 0x0008 | 0x0040 | 0x0080,
      'field'
    );
    ...
    const methodFlags = formatAccessFlags(
      0x0004 | 0x0020 | 0x0100 | 0x0800,
      'method'
    );
